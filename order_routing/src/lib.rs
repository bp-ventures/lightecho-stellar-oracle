...
// Get the token contract client from the `soroban_token_spec.wasm` file.
mod token {
...
#[derive(Clone)]
#[contracttype]
// Represents the offer managed by the SingleOrder contract.
pub enum DataKey {
    Order,
}

// Represents an offer managed by the SingleOrder contract.
...
impl SingleOrder {
    ...
    // See comment above the `Order` struct for information on pricing.
    pub fn create(
        e: Env,
        seller: Address,
        ...
        if e.storage().has(&DataKey::Order) {
    ...
    pub fn get_offer(e: Env) -> Order {
        load_offer(&e)
    }
}

fn load_offer(e: &Env) -> Order {
    e.storage().get_unchecked(&DataKey::Order).unwrap()
}

fn write_offer(e: &Env, offer: &Order) {
    e.storage().set(&DataKey::Order, offer);
}

// get price of liquitidity pool associated with the asset
// fn get_lp_price(e: &Env, asset: &BytesN<32>) -> u32 {
//     let lp = e
//         .storage()
...
//     let lp_price = get_lp_price(&e, &sell_asset);
